...
public:
    ...
    void loadEOSData() {
        ...
        std::cout << "Loaded " << rho.size() << " EOS points" << std::endl;
        ...
        for (size_t i = 0; i < rho.size(); ++i) {
            eos_data.lg_rho[i] = std::log10(rho[i] * Constants::dens_conversion); // Convert to CGS
    ...
    void eosFromPres(double pres_in, double& rho_out, double& edens_out, 
        ...
        rho_out = std::pow(10.0, lg_rho_interp);
    ...
    void eosFromDens(double rho_in, double& pres_out, double& edens_out) {
        double lg_rho_in = std::log10(rho_in);
    ...
    std::vector<std::vector<double>> integrate(double r_start, double r_end, 
        ...
        for (int i = 0; i < num_points; ++i) {
            ...
            if (i < num_points - 1 && y[0] > 1e-10) { // Stop if pressure becomes too small
                // RK4 step
    ...
    double findSurface(double pmin, double rhoc, double rad_high) {
        auto surface_func = [this, pmin, rhoc](double r_max) -> double {
            ...
            double g00 = std::sqrt(1.0 - 2.0 * Constants::CGS_G * m / (r_min * Constants::CGS_C * Constants::CGS_C));
            double m_baryon = m / g00;
            double yp = 2.0;
        ...
        for (int iter = 0; iter < 50; ++iter) {
            double c = (a + b) / 2.0;
            double fa = surface_func(a);
            double fc = surface_func(c);
    ...
    double calcTidalDeformability(double C, double Y) {
        if (C >= 0.5 || Y <= 0) return std::numeric_limits<double>::quiet_NaN();
        
        double log_term = std::log(1.0 - 2.0 * C);
        if (!std::isfinite(log_term)) return std::numeric_limits<double>::quiet_NaN();
        
        double zeta = 4.0 * C * C * C * (13.0 - 11.0 * Y + C * (3.0 * Y - 2.0) + 
                     2.0 * (C * C) * (1.0 + Y)) +
                     3.0 * ((1.0 - 2.0 * C) * (1.0 - 2.0 * C)) * 
                     (2.0 - Y + 2.0 * C * (Y - 1.0)) * log_term +
                     2.0 * C * (6.0 - 3.0 * Y + 3.0 * C * (5.0 * Y - 8.0));
        
        if (std::abs(zeta) < 1e-15) return std::numeric_limits<double>::quiet_NaN();
    ...
    // Simple test of EOS interpolation
    void testEOS() {
        double rhoc = 6e14;
        double pres_c, edens_c;
        eosFromDens(rhoc, pres_c, edens_c);
        std::cout << "Test EOS: rhoc = " << rhoc << ", pres = " << pres_c << ", edens = " << edens_c << std::endl;
    }
    ...
    void solve() {
        ...
        if (pos1 != std::string::npos) {
            eos_key = eos_key.substr(pos1 + 11);
        }
        ...
        // Open output file
        std::ofstream outfile("tov_" + eos_key + "_cpp.txt");
